...
mydata = data %>% filter(!V59 %in% c(-5,-2,-1)) %>%
                  ...
                  mutate(x = ifelse(V59<=5,0,1),age = case_when(V242<=20~"<=20",
                                                                V242>20 & V242<=30 ~"20-30",
                                                                V242>30 & V242<=40 ~"30-40",
                                                                V242>40 & V242<=50 ~"40-50",
                                                                V242>50 & V242<=60 ~"50-60",
                                                                V242>60 & V242<=70 ~"60-70",
                                                                V242>70 & V242<=80 ~"70-80",
                                                                V242>80~">80"),
                         x1 = ifelse(V55<=5,0,1)) %>% 
...
mydata = data %>% filter(!V59 %in% c(-5,-2,-1)) %>%
  filter(!V23 %in% c(-5,-4,-3,-2,-1)) %>% 
  filter(!V240 %in% c(-5,-4,-3,-2,-1)) %>% 
  filter(!V242 %in% c(-5,-4,-3,-2,-1)) %>% 
  filter(!V254 %in% c(-5,-4,-3,-2,-1)) %>% 
  filter(!V55 %in% c(-5,-4,-3,-2,-1)) %>% 
  filter(!V57 %in% c(-5,-4,-3,-2,-1)) %>% 
  filter(!V248_CS %in% c(-5,-4,-3,-2,-1)) %>% 
  #filter(V242 > 40) %>% 
  mutate(x1 = ifelse(V59<=5,0,1),age = case_when(V242>40 & V242<=60 ~"40-60",
                                                V242>60 & V242<=80 ~"60-80",
                                                V242>80~">80"),
         x = ifelse(V55<=5,0,1),
         education = ifelse(V248_CS <= 840108,840108,V248_CS)) %>% 
  select(V2A,V23,x,x1,V240,age,V254,V57,education) %>%
  rename(y=V23,region=V2A,sex=V240,ethnicity=V254,marital_status=V57) 
...
ma.ipd.seq.expr(1:k,k) # 0.01607086
ma.ipd.seq.expr(NULL,0) # 0.01709499
