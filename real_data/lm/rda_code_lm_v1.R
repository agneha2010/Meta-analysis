...
mydata = data %>% filter(!V59 %in% c(-5,-2,-1)) %>%
                  ...
                  mutate(x = ifelse(V59<=5,0,1),age = case_when(V242<=20~"<=20",
                                                                ...
                                                                V242>80~">80")) %>% 
                  select(V2A,V23,x,V240,age) %>%
                  rename(y = V23,region = V2A,sex=V240) 

mydata$region.sex.age = paste(paste0(mydata$region),"_",paste0(mydata$sex),"_",paste0(mydata$age))
code = unique(mydata$region.sex.age)
k = length(code)
data.list = list()
n = c()
...
for(i in 1:k){
  #i=1
  data.list[[i]] = mydata[mydata$region.sex.age==code[i],]
...
mydata$region.sex.age = as.factor(mydata$region.sex.age)
mylm = lm(y~x,data = mydata)
summary(mylm)
mylmer = lmer(y ~ x + (1| region.sex.age),data = mydata)
summary(mylmer)
...
test = optimal.seq(5)  
ma.ipd.seq.expr(1:k,k) # 0.0002031488
ma.ipd.seq.expr(NULL,0) # 0.0002046467
test1 = optimal.seq(2) 
