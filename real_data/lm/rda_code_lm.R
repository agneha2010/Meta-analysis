...
mydata = data %>% filter(!V59 %in% c(-5,-2,-1)) %>%
                  ...
                  mutate(x = ifelse(V59<=5,0,1)) %>% 
                  select(V2A,V23,x) %>%
                  rename(y = V23,region = V2A) 

region.code = unique(mydata$region)
k = length(region.code)
data.list = list()
n = c()
prop.trt.mat = matrix(NA,ncol=2,nrow=k)
for(i in 1:k){
  i=1
  data.list[[i]] = mydata[mydata$region==region.code[i],]
...
summary(mylm)
mylmer = lmer(y ~ x + (1| region),data = mydata)
summary(mylmer)
...
test = optimal.seq(5) # 0.0002027008 
ma.ipd.seq.expr(1:k,k) # 0.0002026079
ma.ipd.seq.expr(NULL,0) # 0.0002027494
test1 = optimal.seq(2) 
