...
m8.maxmin$ma_seq <- rbind(c("1,10"),c("3:4"))
m6.maxmin$ma_seq <- rbind(c("1:2,9:10"),c("2:5"))
m4.maxmin$ma_seq <- rbind(c("1:3,8:10"),c("1:6"))
m2.maxmin$ma_seq <- rbind(c("1:4,7:10"),c("1:8"))
...
ggplot(temp2, aes(x=x,y=rel.eff,color=percent_IPD)) + geom_point(size=3) + geom_line(lwd=1)+ 
  ...
  geom_text(aes(label=ma_seq),hjust=0.7, vjust=1.4,size=5) +
